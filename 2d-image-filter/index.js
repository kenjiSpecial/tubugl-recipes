...
export default class App {
    ...
    _addGui(){
        ...
        this.gui.add(this, '_isGrey').name('isGrey').onChange(()=>{
            this._obj.program.bind();
            this.gl.uniform1f(this._program.getUniforms('uIsGrey').location, this._isGrey);
        });
        this.gui.add(this, '_isFit').name('isFitScreen').onChange(()=>{
            this._obj.program.bind();
            this.gl.uniform1f(this._program.getUniforms('uIsFit').location, this._isFit);
        });
        this.gui.add(this, '_isUVImage').name('isUVImage').onChange(()=>{
            let image = this._isUVImage ? this._uvimage: this._image;
            this._texture.bind().setFilter().wrap().fromImage(image, image.width, image.height);
        });
    ...
    _onload(){
        ...
        this._obj.program.bind();
        this.gl.uniform1f(this._program.getUniforms('uImageRate').location, this._image.height/this._image.width);
    ...
    _startLoad(){
        ...
        this._image.onerror = function(){console.error('image load error');};
        this._image.src = 'dist/' + imageURL;

        this._uvimage = new Image();
        this._uvimage.onerror = function(){console.error('image load error');};
        this._uvimage.src = 'dist/' + uvImageURL;
    }
    ...
    resize(width, height){
        ...
        this._obj.program.bind();
        this.gl.uniform1f(this._program.getUniforms('uWindowRate').location, this._height/this._width);
    }
